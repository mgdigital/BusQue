...
You need to run at least one worker instance for each of your queues, using something like [supervisord](http://supervisord.org/). Thanks to Redis' [brpoplpush](http://redis.io/commands/brpoplpush) method, multiple workers can run on the same queue without concurrency issues.
