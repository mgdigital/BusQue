local queue, start_time, end_time = ARGV[1], ARGV[2], ARGV[3]

local result = redis.call('ZRANGEBYSCORE', ':schedule', start_time, end_time)
...
for i, joined in ipairs(result) do
    ...
    if queue == nil or queue == '' or this_queue == queue then
        num = redis.call('ZREM', ':schedule', joined)
        if num > 0 then
            redis.call('SREM', ':'..this_queue..':reserved_ids', id)
        end
    end
end
